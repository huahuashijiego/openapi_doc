...
名称|必填|类型|说明
---|:-:|:-:|:-:
type|否|int|配置项分类。1：话术，2：线路，3：接口，4：发音人，5: 已删除话术 0：全部（默认）。
pageSize|否|int|分页大小
pageIndex|否|int|页码
...
名称| 必填|类型|说明|备注
---|:-:|:-:|:-:|:-:
task_id|否|string|任务池id|可选，不传时使用默认生成的任务池
...
---|:-:|:-:
total|int|号码总数
task_data_ids|long[]|外呼数据行对应的任务数据编号，用于结果推送数据关联(仅限于只有一轮外呼的情况，如果是多轮外呼，第二轮的ID会发生变化)。
